...
|---|---|---|---|---|
| 0.1 | **R3 — consolidate `log_prior`.** Create ONE canonical `log_prior` function; both existing sites call it. No formula change. | `scripts/enhanced/enh_RURO_draws.py`, `scripts/enhanced/enh_RURO_prep_mnl_basic.py` | high | Recompute `log_prior`/`prior` columns on the certified engine-ready parquets via the consolidated function; **byte-identical** (max abs diff 0.0) to the current stored columns on singles + couples. If any row differs, halt — the two sites had silently diverged and that is a finding. |
| 0.2 | **R1 — fix Box-Cox NumPy gradient.** Fix `box_cox_derivative_theta` (~0.5 off near θ=0) in the NumPy engine. | `scripts/enhanced/estimation_utils.py` | high | Analytical gradient vs central-difference FD < 1e-4 element-wise across θ_c, θ_l grid spanning 0. **No re-estimation** (certified used JAX/CONOPT, not this path) — confirm certified θ̂ unchanged by not touching it. |

**Wave-0 stop condition:** both gates pass; commit in `MNL/` with the gate artifacts. Only then does Wave 1 begin.
